...
var MooTable = new Class({
	...
	initialize: function( el, options ){
		...
		if (this.tableWidth) this.div.setStyle('width',(this.tableWidth+0)+'px');
	...
	_makeTable: function(el){
		...
		if( el.get('tag') == 'table'){
			...
			this.tableWidth = el.getSize().x + 25; // new
	...
	_makeTableFromTable: function(t,count){
		...
		while( count < rows.length && section < this.options.section){
			...
			if (!thead && tr.getElements('th').length > 0){
				...
				var values = [];
				var tdelements = tr.getElements('td');
				if (colcount==0){
				...
				if (colcount>tdelements.length){
					...
					for (var i=0;i<colcount;i++){
						if (rowspans[i]>1){
							values.push( rowspanvalues[i]);
							rs++;
						}
						else {
							var td = tdelements[i-rs];
							values.push( td.innerHTML.stripScripts());
						}
					}
				}
				else {
					tdelements.each( function( td ){ 
						if (td.rowSpan>1){
							rowspan =  td.rowSpan;
						...
						if (tdcount==0){
							try {
								...
								rowspanvalues.push( td.innerHTML.stripScripts()+ " (*)");
							}
							catch (e) {
								rowspanvalues.push( td.innerHTML+ " (*)");
							}
						}
						else {
							rowspanvalues.push( '');
	...
	loadData: function( data, append ){
		if( !this.chk(append) ){append = true;}
		if( !append ){
			this._emptyData();
		}
		this._loadData( data );
	...
	_createTableFramework: function(){
		...
		this.tbody = new Element('div').addClass('tbody').inject( this.thead,'after' );
	...
	_setHeaderWidth: function(){
		...
		//width = Math.min(this.tableWidth, width+20)
		this.thead_tr.setStyle('width', width+20+'px');
		this.thead_tr.getParent().setStyle('width', width+20+'px');// new
		this.tableWidth = Math.min(this.tableWidth, width+20);
	},
	...
	_setWidths: function(){
		...
		if (this.tableWidth  < this.thead_tr.getSize().x){
			this.mootable.setStyle( 'width', this.tableWidth+"px");
			//this.mootable.setStyle("overflow-x","auto");
		}
	},
	...
	_initDisplayOptions: function(){
		...
		this.displayOptions.addClass('displayOptions').inject( this.displayOptionsTrigger,'after' );
	},
	...
	sort: function( col ){
		...
		if( this.chk(this.sortby) ){
			this.headers[this.sortby].element.removeClass( 'sorted_'+ (this.sortasc ? 'asc' : 'desc' ) );
		}
		if( this.chk(this.sortby) && this.sortby == col ){
			this.sortasc = !this.sortasc;
		}
		else if( this.chk(this.sortby) ){
