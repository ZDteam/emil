...
require_once( EBLOG_CLASSES . DIRECTORY_SEPARATOR . 'themes.php' );
...
/*]]>*/";
			...
			$document->addScript( EBLOG_MEDIA_URI . (($easyblogEnvironment=='development') ? 'scripts_/' : 'scripts/') . 'abstract.js' );
	...
	public static function getLoginLink( $returnURL = '' )
	{
		...
		switch( $config->get( 'main_login_provider' ) )
		{
			...
			case 'jomsocial':
				...
				if( EasyBlogHelper::getJoomlaVersion() >= '1.6' )
				{
					$link 	= EasyBlogRouter::_('index.php?option=com_users&view=login' . $returnURL );
				}
				else
				{
					$link	= EasyBlogRouter::_( 'index.php?option=com_user&view=login' . $returnURL );
				}
			break;
		}
