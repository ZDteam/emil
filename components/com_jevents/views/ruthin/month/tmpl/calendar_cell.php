	...
	function calendarCell(&$currentDay,$year,$month,$i, $slot){
		...
		if( $currentDay['countDisplay'] < $cfg->get('com_calMaxDisplay',5)){
			...
			if ($templatedcell){
				...
				if ($lang->isRTL())	$cellStyle .= "border-right-color:$bgeventcolor;border-bottom-color:$bgeventcolor;";
				else $cellStyle .= "border-left-color:$bgeventcolor;border-bottom-color:$bgeventcolor;";
			}
		}else{
