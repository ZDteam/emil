...
for ($dn=0;$dn<$datacount;$dn++){
	...
	for ($i=0;$i<count($this->data["dates"][$dn]["events"]);$i++){
		...
		if ( $event->endDate()!= $event->startDate() && $event->multiday() && !isset($event->_length)) {
			...
			if ($cfg->get('eptruncpriorpost',0) && $currentDay["d"]==1 && $currentDay["cellDate"]>JevDate::strtotime($event->startDate())){
			...
			if ($currentDay["cellDate"]==$event->_startday || ($cfg->get('eptruncpriorpost',0) && $currentDay["d"]==1) ) {
				...
				for ($block=1;$block<$blocks;$block++) {
					if ($cfg->get('eptruncpriorpost',0) && $this->data["dates"][$dn+$block]["month"] != $this->month) {
