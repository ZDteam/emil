		...
		if( $isNew && $this->published == 1 && $state )
		{
			...
			if( $blog->created_by != $this->created_by && $this->created_by != 0 )
			{
				...
				if( $config->get( 'integrations_jomsocial_notification_comment' ) )
				{
					...
					EasyBlogHelper::getHelper( 'JomSocial' )->addNotification( JText::sprintf( 'COM_EASYBLOG_JOMSOCIAL_NOTIFICATIONS_NEW_COMMENT_IN_YOUR_BLOG' , $blog->title ) , 'easyblog_comment_blog' , array( $blog->created_by ) , $this->created_by , $link );
				}
			}
			...
			if( $config->get( 'integrations_jomsocial_notification_comment_follower' ) && $this->created_by != 0 )
			{
				...
				{
					// Get list of users who subscribed to this blog.
					EasyBlogHelper::getHelper( 'JomSocial' )->addNotification( JText::sprintf( 'COM_EASYBLOG_JOMSOCIAL_NOTIFICATIONS_NEW_COMMENT_POSTED' , $blog->title ) , 'easyblog_comment_blog_sub' , $target , $this->created_by , $link );
				}
			}
