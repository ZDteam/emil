	...
	public function store( $log = true )
	{
		...
		if( $source == 'easyblog' && $state && $this->published == POST_ID_PUBLISHED && $isNew && $log )
		{
			...
			if( $config->get( 'integrations_jomsocial_notification_blog' ) )
			{
				...
				EasyBlogHelper::getHelper( 'JomSocial' )->addNotification( JText::sprintf( 'COM_EASYBLOG_JOMSOCIAL_NOTIFICATIONS_NEW_BLOG' , $author->getName() , $this->title ) , 'easyblog_new_blog' , $target , $this->created_by , $link );
			}
