...
class JFormFieldJevrimage extends JevrField
{
	/**
	* Element name
	*
	...
	*/
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		JPlugin::loadLanguage( 'plg_jevents_jevfiles',JPATH_ADMINISTRATOR );
		...
		if (!isset($setscripts)){
			...
			function setImageFileName(){			
				...
				img.src = '". JEVP_MEDIA_BASEURL."/$folder/thumbnails/thumb_'+iframe.filename;
		...
		$result .= '<input type="button" onclick="clearImageFile(\''.$fieldname.'\')" value="'.JText::_("jev_Delete").'"/>';
