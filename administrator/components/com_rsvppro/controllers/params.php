...
class AdminParamsController extends JControllerAdmin
{
	...
	function __construct($default = array())
	{
		...
		{
			JFactory::getApplication()->redirect("index.php?option=" . RSVP_COM_COMPONENT . "&task=cpanel.cpanel", "Not Authorised - must be admin");
			return;
		}
	...
	function edit()
	{
		...
		if (!$table->load(array("element" => "com_rsvppro", "type" => "component"))) // 1.6 mod
		{
	...
	function save()
	{
		...
		$option = RSVP_COM_COMPONENT;
		$comp = JComponentHelper::getComponent(RSVP_COM_COMPONENT);
		$id = $comp->id;
		...
		if ($return === false)
		{
			...
			$this->setRedirect(JRoute::_('index.php?option=' . RSVP_COM_COMPONENT . '&task=params.edit', false));
			return false;
		}
		...
		switch ($this->getTask()) {
			case 'apply':
				$this->setRedirect('index.php?option=' . RSVP_COM_COMPONENT . '&task=params.edit', JText::_('RSVP_CONFIG_SAVED'));
				break;
			default:
				$this->setRedirect('index.php?option=' . RSVP_COM_COMPONENT . "&task=cpanel.cpanel", JText::_('RSVP_CONFIG_SAVED'));
				break;
		}
		//$this->setRedirect( 'index.php?option='.RSVP_COM_COMPONENT."&task=cpanel.cpanel", JText::_( 'RSVP_CONFIG_SAVED' ) );
		//$this->setMessage(JText::_( 'RSVP_CONFIG_SAVED' ));
		//$this->edit();
