 ...
 *
 * @version     $Id: jevmenu.php 2522 2011-09-01 10:04:51Z geraintedwards $
 * @package     JEvents
...
class JElementJevmenu extends JElement
{
	...
	*/
	...
	function fetchElement($name, $value, &$node, $control_name, $raw=false)  // RSH 10/20/10 - Added raw parameter to return just the elements for J!1.6
	{
		...
		$options 	= array();
		$options[]	= JHTML::_('select.option', '', '- '.JText::_('COM_JEVPEOPLE_SELECT_ITEM').' -');
		...
		$coremenuItems =  $menu->getMenu();
		// must make a copy otherwise by changing the referenced original 
		// it affects the menu modules !!!
		$menuItems = array();
		foreach ($coremenuItems as $coreitem) {
		 	$item = clone $coreitem;
		...
		if ($menuItems)
		{
			...
			{
				$pt 	= 0; // RSH 10/4/10 in J!1.5 - parent was always 0, this changed in J!.16 to a real parent_id, so force id to 0 for compatibility instead of $v->parent;
				$list 	= @$children[0] ? $children[0] : array();
				array_push( $list, $v );
				$children[0] = $list;  // RSH use '0' instead of $pt
			}
		}
