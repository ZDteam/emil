...
class JElementJevpcategory extends JElement
{
	/**
	* Element name
	*
	...
	*/
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$query = 'SELECT c.id, c.title as ctitle,p.title as ptitle, gp.title as gptitle, ggp.title as ggptitle, ' .
				...
				' WHERE c.published = 1 ' .
				' AND c.section LIKE "com_jevpeople_type%"' .
		...
		array_unshift($options, JHTML::_("select.option","0",JText::_( 'NO_CATEGORY' ),"id","title"));
