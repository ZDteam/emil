...
class AdminCategoriesViewCategories extends JViewLegacy
{
	...
	function overview($tpl = null)
	{
		...
		if (JVersion::isCompatible("1.6.0")){
			JHTML::stylesheet('eventsadmin16.css', 'administrator/components/'.JEV_COM_COMPONENT.'/assets/css/');
		}
		else {
		...
		JSubMenuHelper::addEntry(JText::_( 'CONTROL_PANEL' ), 'index.php?option='.JEVEX_COM_COMPONENT, true);
		...
		$limitstart = intval( JFactory::getApplication()->getUserStateFromRequest( "cat_{com_jevpeople}limitstart", 'limitstart', 0 ));
		$catfilter	= intval( JFactory::getApplication()->getUserStateFromRequest( "cat_catid", 'catid', 0 ));
	...
	function catFilter(){
		...
		return JText::_( 'TOP_LEVEL_CATEGORY' ) . " : ". JHTML::_('select.genericlist', $options, 'catid', 'class="inputbox" size="1" id="catid" onchange="form.submit();"', 'value', 'text', $this->catfilter);
	}
	...
	function typeFilter($asinput = false, $javascript = ""){
		...
		if (!$asinput) 	return JText::_( 'PEOPLE_TYPE' ) . " : ". JHTML::_('select.genericlist', $options, 'type_id', 'class="inputbox" size="1" onchange="document.getElementById(\'catid\').value=-1;form.submit();"', 'value', 'text', $typefilter);
